...
def get_dependency_prompt(with_code, coordinator, task_id):
    ...
    if len(task_dependency) > 0:
        dependency_analysis = ""
        if task_id - 1 < len(coordinator.task_dependency_analysis):
            dependency_analysis = coordinator.task_dependency_analysis[task_id - 1]
        dependency_prompt = f"""\
This task is Task {task_id}, which depends on the following tasks: {task_dependency}. The dependencies for this task are analyzed as follows: {dependency_analysis}
"""
        for id in task_dependency:
            task_memory = coordinator.memory.get(str(id), {})
            code_structure = coordinator.code_memory.get(str(id)) or {
                "script_path": "",
                "class": [],
                "function": [],
                "file_outputs": [],
...
{task_memory.get('task_description', '')}
# The modeling method for Task {id}:
{task_memory.get('mathematical_modeling_process', '')}
"""
            if with_code:
                dependency_prompt += f"""\
...
{task_memory.get('solution_interpretation', '')}
---
"""
                dependent_file_prompt += f"""\
# The files generated by code for Task {id}:
{code_structure.get('file_outputs', [])}
"""
            else:
                dependency_prompt += f"""\
# The result for Task {id}:
